...
def train(df, target, hparam):
    ...
    #y = y_scaler.fit_transform(df[target].values.reshape(-1,1))
        df_sol = pd.read_csv("extended_big_additive_category.csv")
        fe_sol=["param1"]
    #y_unscaled_1=y_scaler.fit_transform(df_sol[fe_sol].values.reshape(-1,1))
        y_unscaled_feat_1=(df_sol[fe_sol].values.reshape(-1,1))
        fe_sol=["param2"]
        y_unscaled_feat_2=(df_sol[fe_sol].values.reshape(-1,1))
        fe_sol=["param3"]
        y_unscaled_feat_3=(df_sol[fe_sol].values.reshape(-1,1))
        fe_sol=["param4"]
        y_unscaled_feat_4=(df_sol[fe_sol].values.reshape(-1,1))
        fe_sol=["param5"]
        y_unscaled_feat_5=(df_sol[fe_sol].values.reshape(-1,1))
        fe_sol=["param6"]
        y_unscaled_feat_6=(df_sol[fe_sol].values.reshape(-1,1))
        fe_sol=["param7"]
        y_unscaled_feat_7=(df_sol[fe_sol].values.reshape(-1,1))
        fe_sol=["param8"]
        y_unscaled_feat_8=(df_sol[fe_sol].values.reshape(-1,1))
        fe_sol=["param9"]
        y_unscaled_feat_9=(df_sol[fe_sol].values.reshape(-1,1))
        fe_sol=["param10"]
        y_unscaled_feat_10=(df_sol[fe_sol].values.reshape(-1,1))

        fe_sol=["temperature"]
        y_unscaled_feat_11=(df_sol[fe_sol].values.reshape(-1,1))

        fe_sol=["time"]
        y_unscaled_feat_12=(df_sol[fe_sol].values.reshape(-1,1))

        fe_sol=["additive_category"]
        y_unscaled_feat_13=(df_sol[fe_sol].values.reshape(-1,1))


        y_unscaled=np.hstack([y_unscaled_feat_1,y_unscaled_feat_2,y_unscaled_feat_3,y_unscaled_feat_4,y_unscaled_feat_5,y_unscaled_feat_6,y_unscaled_feat_7,y_unscaled_feat_8,y_unscaled_feat_9,y_unscaled_feat_10,y_unscaled_feat_11,y_unscaled_feat_12,y_unscaled_feat_13])
...
###############################################################
    ...
    #model=MLPRegressor(random_state=1, max_iter=500)
        ...
        model = getmodel(x_train.shape[-1],sid)
        print (model)
        y_pred_train,y_pred_test=model_training(model_name,model,x_train,y_train,x_test,y_test,sid) 
        #model =  MLPRegressor(hidden_layer_sizes=(50,20,10),activation='relu',tol=1e-10, batch_size=n_sample, max_iter=200)
        ...
        np.savetxt("%s/predictions_rawdata/y_NNR_test.txt"%(outdir), y_pred_test_unscaled)
        np.savetxt("%s/predictions_rawdata/y_NNR_train.txt"%(outdir), y_pred_train_unscaled)
